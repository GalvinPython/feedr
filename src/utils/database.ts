...
export async function initTables(): Promise<boolean> {
    ...
    const createTikTokTable = `
        CREATE TABLE IF NOT EXISTS tiktok (
            tiktok_user_id VARCHAR(255) NOT NULL PRIMARY KEY,
            latest_video_id VARCHAR(255) UNIQUE
        );
    `;
    ...
    const createBotInfoTable = `
        CREATE TABLE IF NOT EXISTS bot_info (
            total_servers INT NOT NULL DEFAULT 1,
    ...
    pool.query(createTikTokTable, (err) => {
        if (err) {
            console.error("Error creating TikTok table:", err);
            return false
        } else {
            console.log("TikTok table created");
        }
    });
    pool.query(createBotInfoTable, (err) => {
        if (err) {
            console.error("Error creating Bot Info table:", err);
            return false
        } else {
...
export async function updateBotInfo(total_servers: number, total_members: number) {
    console.log("Updating bot info:", total_servers, total_members);
    const query = `UPDATE bot_info SET total_servers = ?, total_members = ?`;
    return new Promise<boolean>((resolve, reject) =>
        pool.query(query, [total_servers, total_members], (err) => {
            if (err) {
